    ...
    return
        ['SETTINGS' => [ 											// Настройки обработчика
            ...
            'CHECKOUT_DATA' => [									// Настройки формы
                'ACTION_URI' => 'https://b24.ecomkassa.ru/pay.php', 	// URL, на который будет отправляться форма
...
function GetPayUrl( $token, $kassaid, $paymentsType, $email, $totalSumm, $arrayItems, $companyArray, $externalId, $secret ){
    ...
    if ($curl = curl_init()) {
        ...
        $jayParsedAry = [
            ...
            "service" => [
                "callback_url" => "https://b24.ecomkassa.ru/callback.php?secret=$secret&externalId=$externalId"
            ],
