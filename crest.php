<?php
	 ...
	 *      C_REST_MYSQL_DBNAME = dbData
	 *      C_REST_MYSQL_USERNAME = dbData
	 *      C_REST_MYSQL_PASSWORD = dbData
	 */
	...
	class CRest
	{
		 ...
		 *
		 */
		protected static function callCurl($arParams, $memberId)
		{
			...
			if($arSettings !== false)
			{
				...
				try
				{
					...
					if(!empty($result[ 'error' ]))
					{
						...
						{
							$result = static::GetNewAuth($arParams, $memberId);
						}
						else
						{
							$arErrorInform = [
		...
		public static function call($method, $memberId, $params = [])
		{
			$arPost = [
				'method' => $method,
				'params' => $params
			...
			$result = static::callCurl($arPost, $memberId);
			return $result;
		}
		...
		public static function callBatch($memberId, $arData, $halt = 0)
		{
			...
			if(is_array($arData))
			{
				...
				if(!empty($arDataRest))
				{
					...
					$arResult = static::callCurl($arPost, $memberId);
				}
			}
			return $arResult;
		}
		...
		private static function GetNewAuth($arParams, $memberId)
		{
			$result = [];
			$arSettings = static::getAppSettings( $memberId );
			if($arSettings !== false)
			{
				...
				$newData = static::callCurl($arParamsAuth, $memberId);
				...
				if(static::setAppSettings($newData, $memberId ))
				{
					$arParams[ 'this_auth' ] = 'N';
					$result = static::callCurl($arParams, $memberId);
				}
			}
			return $result;
		}
		...
		private static function setAppSettings($arSettings, $memberId, $isInstall = false )
		{
			$return = false;
			if(is_array($arSettings))
			{
				$oldData = static::getAppSettings( $memberId );
		 ...
		 *
		 * @return array setting for getAppSettings( $memberId )
         *
		 */
		...
		protected static function getSettingData( $memberId )
		{
			...
			if( $userData['id'] > 0 && $memberId != '-' )//Костыль -
			{
				$return = static::expandData( $userData['settings'] );
		...
		protected static function setSettingData($arSettings)
        {
            ...
            if ( $arSettings['member_id'] && $arSettings['member_id'] != '-' ) {
                $stmt = $db->prepare("SELECT `id` FROM users WHERE `member_id` = ?");
                $stmt->execute([$arSettings['member_id']]);
                $userData = $stmt->fetch(PDO::FETCH_LAZY);
                if( $userData['id'] > 0 && $arSettings['member_id'] != '-' )
                {
                    $query = "UPDATE `users` SET `settings` = :sett WHERE `id` = :id";
                    $params = [
                        ':id' => $userData['id'],
                        ':sett' => static::wrapData($arSettings)
                    ];
                    $stmt = $db->prepare($query);
                    //обновление
                }else{
                    $secret = md5(rand(1, 10000000).time());
                    $query = "INSERT INTO `users` (`settings`, `member_id`, `unix_install`, `secret_code`) VALUES (:sett,:memb,:unix,:secret)";
                    $params = [
                        ':sett' => static::wrapData($arSettings),
                        ':memb' => $arSettings['member_id'],
                        ':unix' => time(),
                        ':secret' => $secret
                    ];
                    $stmt = $db->prepare($query);
